...
class ssl_server : public tcp_service_base, public ssl_service {
	...
	void start(uint16_t port, bool reuse_addr = true, int listen_backlog = DEFAULT_BACKLOG, ip v = ip::v4) noexcept {
		try {
			...
			if (!is_running()) {
				...
				_acceptor_context_ptr->accept(_new_channel->socket(), std::bind(&ssl_server::accept_complete, this, std::placeholders::_1, std::placeholders::_2));
			}
...
private:
	...
	virtual void accept_complete(const asio::error_code& error, const asio::ip::tcp::endpoint& peer_endpoint) {
		if (error) {
			...
			auto ch_ctx_ptr = std::make_shared<ssl_channel_context>(std::move(_new_channel), this);
			...
			if (!ret.second) { // not inserted. already exist. this is a critical error!
				...
				pair.second->handshake(asio::ssl::stream_base::server, std::bind(&ssl_server::handshake_complete, this, std::placeholders::_1, peer_endpoint, ch_ctx_ptr));
			}
		}
	...
	void handshake_complete(const asio::error_code& error, const asio::ip::tcp::endpoint& peer_endpoint, ssl_channel_context_ptr ch_ctx_ptr) {
		if (error)
