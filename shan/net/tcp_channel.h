...
//		assert(_write_buf_queue.front() == write_sb_ptr);
		...
		if (!_write_buf_queue.empty()) {
			...
			asio::async_write(socket(), asio::buffer(next_buf_ptr->in_ptr(), next_buf_ptr->in_size()),
							  std::bind(&tcp_channel::write_complete, this, std::placeholders::_1, std::placeholders::_2, next_buf_ptr, write_handler));
		}
	}
