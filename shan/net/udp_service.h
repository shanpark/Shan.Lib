...
class udp_service : public service {
public:
	...
	void bind_connect(uint16_t local_port, const std::string& remote_address = std::string(""), uint16_t remote_port = 0, ip v = ip::v4) {
		if (is_running()) {
			...
			std::pair<std::size_t, channel_context_ptr> pair = std::make_pair(ch_ctx_ptr->channel_id(), ch_ctx_ptr);
...
private:
	...
	void resolve_complete(const asio::error_code& error, asio::ip::udp::resolver::iterator it, channel_context_ptr ch_ctx_ptr) {
		if (error)
	...
	virtual void fire_channel_read(channel_context_ptr ch_ctx_ptr, util::streambuf_ptr& sb_ptr) {
		auto ep = static_cast<udp_channel_context*>(ch_ctx_ptr.get())->get_sender();
		ip_port ip(ep.address(), ep.port());

		ch_ctx_ptr->strand().post([this, ch_ctx_ptr, sb_ptr, ip]() { //
			ch_ctx_ptr->done(false); // reset context to 'not done'.
	...
	void fire_channel_write_to(channel_context_ptr ch_ctx_ptr, object_ptr data, const ip_port& destination) {
		ch_ctx_ptr->strand().post([this, ch_ctx_ptr, data, destination]() {
			ch_ctx_ptr->done(false); // reset context to 'not done'.
			...
			if (sb_ptr)
				static_cast<udp_channel_context*>(ch_ctx_ptr.get())->write_streambuf_to(sb_ptr, destination, std::bind(&udp_service::write_complete, this, std::placeholders::_1, std::placeholders::_2, std::placeholders::_3, ch_ctx_ptr));
			else
