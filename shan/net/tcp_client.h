...
private:
	...
	virtual void new_channel_connected(tcp_channel_context_base_ptr ch_ctx_ptr) override {
		ch_ctx_ptr->call_channel_connected(ch_ctx_ptr->strand().wrap(std::bind(&tcp_client::read_complete, this, std::placeholders::_1, std::placeholders::_2, ch_ctx_ptr)));
	}
	...
	virtual void resolve_complete(const asio::error_code& error, asio::ip::tcp::resolver::iterator it, tcp_channel_context_base_ptr ch_ctx_ptr) override {
		if (error) {
			...
			static_cast<tcp_channel_context*>(ch_ctx_ptr.get())->connect(it, ch_ctx_ptr->strand().wrap(std::bind(&tcp_client::connect_complete, this, std::placeholders::_1, std::placeholders::_2, ch_ctx_ptr)));
		}
	}
};
