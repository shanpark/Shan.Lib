...
class tcp_client : public tcp_service_base {
public:
	...
	void connect(const std::string& address, uint16_t port) {
		if (is_running()) {
			...
			_resolver_ptr->async_resolve(query, std::bind(&tcp_client::resolve_complete, this, std::placeholders::_1, std::placeholders::_2, ch_ctx_ptr));
		}
	}
	...
	void resolve_complete(const asio::error_code& error, asio::ip::tcp::resolver::iterator it, tcp_channel_context_ptr ch_ctx_ptr) {
		if (error) {
			...
			if (!ret.second) { // not inserted. already exist. this is a critical error!
				...
				ch_ctx_ptr->connect(it->endpoint().address().to_string(), it->endpoint().port(), std::bind((&tcp_client::connect_complete), this, std::placeholders::_1, ch_ctx_ptr)); // try first endpoint unconditionally.
			}
		}
	}
