...
void bytes_pack_test() {
	std::cout << "8. MsgPack test (byte vector)";
...
void streambuf_pack_test() {
	std::cout << "9. MsgPack test (streambuf)";
	...
	try {
		...
		shan::util::streambuf packed;
		js.pack(packed);
		if (packed.in_size() == 18)
			std::cout << "[OK]";
		else
			std::cout << "[Fail]";
