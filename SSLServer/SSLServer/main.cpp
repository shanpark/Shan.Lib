...
public:
	...
	virtual void channel_connected(tcp_channel_context_base* ctx) override {
		{
			std::lock_guard<std::mutex> _lock(_mutex);
			cout << static_cast<ssl_channel_context*>(ctx)->channel_id() << ":serv_ch_handler::" << "channel_connected(" << ctx->channel_id() << ") called" << endl;
			conn++;
		}
	...
	virtual void channel_read(tcp_channel_context_base* ctx, shan::object_ptr& data) override {
		std::lock_guard<std::mutex> _lock(_mutex);
		cout << static_cast<ssl_channel_context*>(ctx)->channel_id() << ":serv_ch_handler::" << "channel_read()" << endl;
	}

	virtual void channel_rdbuf_empty(tcp_channel_context_base* ctx) override {
		std::lock_guard<std::mutex> _lock(_mutex);
		cout << static_cast<ssl_channel_context*>(ctx)->channel_id() << ":serv_ch_handler::" << "channel_rdbuf_empty() called" << endl;
	}
	...
	virtual void channel_write(tcp_channel_context_base* ctx, shan::object_ptr& data) override {
		{
			std::lock_guard<std::mutex> _lock(_mutex);
			auto sb_ptr = static_cast<shan::util::streambuf*>(data.get());
			cout << static_cast<ssl_channel_context*>(ctx)->channel_id() << ":serv_ch_handler::" << "channel_write() - " << sb_ptr->in_size() << endl;
		}
	}
